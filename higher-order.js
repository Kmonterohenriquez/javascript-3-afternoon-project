...
//Code Here
let evenNumbers = mixedNumbers.filter((curr)=> curr%2 ===0);// = mixedNumbers.filter(/* Provide Your Callback Here */)
...
//Code Here
let postTaxPrices = prices.map((curr)=>curr * 1.07);// = prices.map(/* Provide Your Callback Here );
...
//Code Here
let myStrongest = monstersInYourPocket.filter((curr)=> curr.CP > 200);// = monstersInYourPocket.filter(/* Provide Your Callback Here */)
...
let orderTotals = orders.map((curr)=> (curr.price + curr.tax) * 1.0834 ); // Code here
// let
...
let bobPurchases = purchases.filter((curr)=> curr.owner === 'Bob');
let bobsTotal= bobPurchases.reduce((acc, curr)=> acc + curr.price, 0); //Code Here
